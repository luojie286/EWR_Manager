...
本地存储的个人动漫收藏与感想管理工具，类似 Bangumi / MyAnimeList / AniList 的个人离线版本。
...
| 表名 | 字段 |
|------|------|
| `anime` | id, title, score, status, description, cover_path |
| `review` | id, anime_id, date, title, content |
| `tag` | id, name |
| `anime_tag` | anime_id, tag_id |

数据库文件位置：`%APPDATA%/Personal/AnimeManager/anime.db`
...
本机已检测到 Qt 6.10.2（`C:\Qt\6.10.2`）。

### 方式一：Qt Creator（推荐）

1. 打开 Qt Creator
2. **文件 → 打开文件或项目**，选择 `e:\myproject\CMakeLists.txt`
...
>>>>>>> 4d8ad6a206cc0bb5e78e039a8f8cb1af69dd9cc1
